...
async def forward_geocode(landmark: str = "", city: str = "", district: str = "", state: str = "", pincode: str = "") -> dict:
    ...
    All arguments are optional keyword args so callers can supply only what they have.
    """
    SEARCH_URL = "https://nominatim.openstreetmap.org/search"
    
    # Try multiple queries from most specific to least specific
    queries = [
        ", ".join([p for p in [landmark, district, city, state, pincode, "India"] if p]),
        ", ".join([p for p in [district, city, state, pincode, "India"] if p]),
        ", ".join([p for p in [city, pincode, "India"] if p]),
